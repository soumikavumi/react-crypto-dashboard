    ...
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="button" onClick={() => onSort("market_cap")}>Sort: Mkt Cap{sortBy === "market_cap" ? " •" : ""}</button>
          <button className="button" onClick={() => onSort("price")}>Sort: Price{sortBy === "price" ? " •" : ""}</button>
          <button className="button" onClick={() => onSort("change")}>Sort: 24h%{sortBy === "change" ? " •" : ""}</button>
        </div>
      </div>
          ...
          {coins.map((c) => {
            ...
            return (
              <tr key={c.id}>
                ...
                <td>
                  <Link to={`/coin/${c.id}`} style={{ display: "flex", alignItems: "center", gap: 8 }}>
